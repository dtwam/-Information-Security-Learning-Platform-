...
import { labChallenges } from "@/data/courseData";
...
export default function LabPage() {
  ...
  const challenge = labChallenges[activeChallenge];
  const difficultyColors = {
    beginner: "text-cyber-success bg-cyber-success/10",
    intermediate: "text-cyber-warning bg-cyber-warning/10",
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-2">
            <h2 className="font-display font-semibold mb-3 text-sm text-muted-foreground uppercase tracking-wider">
              Challenges
            </h2>
            {labChallenges.map((ch, i) => (
              <button
                key={ch.id}
                onClick={() => { setActiveChallenge(i); setShowHints(false); }}
                className={`w-full text-left p-4 rounded-xl transition-colors ${
                  i === activeChallenge ? "glass cyber-glow" : "glass hover:bg-secondary/80"
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {progress.completedChallenges.includes(ch.id) && (
                ...
                <span className={`text-xs px-2 py-0.5 rounded-md mt-1 inline-block ${difficultyColors[ch.difficulty]}`}>
                  {ch.difficulty}
                </span>
              </button>
            ))}
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-md ${difficultyColors[challenge.difficulty]}`}>
              ...
              <div className="space-y-2 mb-4">
                <h3 className="text-sm font-semibold flex items-center gap-1">
                  <AlertTriangle className="w-3.5 h-3.5 text-primary" /> Instructions
                </h3>
                {challenge.instructions.map((inst, i) => (
                  <p key={i} className="text-sm text-muted-foreground pl-5">
                    {i + 1}. {inst}
                  </p>
                ))}
              </div>
              ...
              {showHints && (
                <div className="bg-primary/5 rounded-xl p-3 mb-4 space-y-1">
                  {challenge.hints.map((hint, i) => (
                    <p key={i} className="text-sm text-primary">💡 {hint}</p>
                  ))}
                </div>
              )}
